...
import noblecrm
import noblecrm.modules.registry
from noblecrm.api import call_kw, Environment
from noblecrm.modules import get_resource_path
from noblecrm.tools import crop_image, topological_sort, html_escape, pycompat
from noblecrm.tools.translate import _
from noblecrm.tools.misc import str2bool, xlwt, file_open
from noblecrm.tools.safe_eval import safe_eval
from noblecrm import http
from noblecrm.http import content_disposition, dispatch_rpc, request, \
    serialize_exception as _serialize_exception, Response
from noblecrm.exceptions import AccessError, UserError
from noblecrm.models import check_method_name
from noblecrm.service import db
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('noblecrm.addons.web', "views")
    ...
    try:
        ...
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, noblecrm.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = noblecrm.conf.server_wide_modules or ['web']
    serverside = []
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("noblecrm.addons.web.main.manifest_list(): debug parameter is deprecated")
...
def fix_view_modes(action):
    """ For historical reasons, NobleCRM has weird dealings in relation to
...
def _local_web_translations(trans_file):
    ...
    try:
        ...
        return
    for x in po:
        if x.id and x.string and "noblecrmerp-web" in x.auto_comments:
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = noblecrm.SUPERUSER_ID

        values = request.params.copy()
        try:
            values['databases'] = http.db_list()
        except noblecrm.exceptions.AccessDenied:
        ...
        if not noblecrm.tools.config['list_db']:
...
class WebClient(http.Controller):
    ...
    def translations(self, mods=None, lang=None):
        ...
        messages = request.env['ir.translation'].sudo().search_read([
            ...
            ('comments', 'like', 'noblecrmerp-web'), ('value', '!=', False),
            ('value', '!=', '')],
    ...
    def version_info(self):
        return noblecrm.service.common.exp_version()
...
class Proxy(http.Controller):
    ...
    def post(self, path):
        ...
        with request.session.load_request_data() as data:
            ...
            headers = {'X-Noblecrmerp-Session-Id': request.session.sid}
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = noblecrm.tools.config.verify_admin_password('admin')
        d['list_db'] = noblecrm.tools.config['list_db']
        d['langs'] = noblecrm.service.db.exp_list_lang()
        d['countries'] = noblecrm.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = noblecrm.service.db.list_db_incompatible(d['databases'])
        except noblecrm.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        try:
            noblecrm.service.db.check_super(master_pwd)
            ...
            dump_stream = noblecrm.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=noblecrm.SUPERUSER_ID))
    ...
    def account(self):
        ...
        return 'https://accounts.infonoble.com/oauth2/auth?' + werkzeug.url_encode(params)
...
class Binary(http.Controller):
    ...
    def content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        if crop and (width or height):
            ...
            if height > 500:
                height = 500
            content = noblecrm.tools.image_resize_image(base64_source=content, size=(width or None, height or None), encoding='base64', filetype='PNG')
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = noblecrm.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: noblecrm.tools.ustr(field[1].get('string', '')))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from NobleCRM's export data to whatever the
