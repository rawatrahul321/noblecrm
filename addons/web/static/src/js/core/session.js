...
var Session = core.Class.extend(mixins.EventDispatcherMixin, {
    ...
    init: function (parent, origin, options) {
        ...
        options = options || {};
        this.module_list = (options.modules && options.modules.slice()) || (window.noblecrm._modules && window.noblecrm._modules.slice()) || [];
        this.server = null;
    ...
    on_modules_loaded: function () {
        var noblecrmerp = window.noblecrmerp;
        for(var j=0; j<this.module_list.length; j++) {
            ...
            noblecrmerp[mod] = {};
            // init module mod
            var fct = noblecrmerp._noblecrmerp[mod];
            if(typeof(fct) === "function") {
                noblecrmerp._noblecrmerp[mod] = {};
                for (var k in fct) {
                    noblecrmerp._noblecrmerp[mod][k] = fct[k];
                }
                fct(noblecrmerp, noblecrmerp._noblecrmerp[mod]);
            }
    ...
    session_reload: function () {
        var result = _.extend({}, window.noblecrm.session_info);
        delete result.session_id;
        ...
        return self.check_session_id().then(function () {
            ...
            if (self.origin_server) {
                ...
                if (self.override_session) {
                    options.headers["X-Noblecrmerp-Session-Id"] = self.session_id || '';
                }
