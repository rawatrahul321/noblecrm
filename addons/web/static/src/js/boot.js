/*---------------------------------------------------------
 * NobleCRMerp Web Boostrap Code
 *---------------------------------------------------------*/

/**
 * @name noblecrmerp
 * @namespace noblecrmerp
 *
...
(function() {
    ...
    var noblecrm = window.noblecrm = window.noblecrm || {};
    _.extend(noblecrm, {
            ...
            if (jobs.length) {
                ...
                for (var k=0; k<jobs.length; k++) {
                    debug_jobs[jobs[k].name] = job = {
                        ...
                        dependents: noblecrm.__DEBUG__.get_dependents(jobs[k].name),
                        name: jobs[k].name
                    };
                    if (jobs[k].error) {
                    ...
                    var deps = noblecrm.__DEBUG__.get_dependencies( job.name );
                ...
                missing = noblecrm.__DEBUG__.get_missing_jobs();
                failed = noblecrm.__DEBUG__.get_failed_jobs();
                var unloaded = _.filter(debug_jobs, function (job) { return job.missing; });

                if (noblecrm.debug || !_.isEmpty(failed) || !_.isEmpty(unloaded)) {
                    ...
                    if (noblecrm.debug && !_.isEmpty(debug_jobs)) {
                        log('Debug:                   ', debug_jobs);
                    }
                }
            }
            noblecrm.__DEBUG__.js_modules = {
                missing: missing,
                failed: _.pluck(failed, 'name'),
            };
            noblecrm.__DEBUG__.didLogInfo.resolve();
        },
                ...
                if (!job.error) {
                    $.when(job_exec).then(
                        function (data) {
                            ...
                            def.resolve();
                            noblecrm.process_jobs(jobs, services);
                        }, function (e) {
                            job.rejected = e || true;
                            jobs.push(job);
                            def.resolve();
