noblecrm.define('web.data', function (require) {
"use strict";
 ...
 * consumed by NobleCRMerp's RPC APIs.
 *
...
var Query = Class.extend({
     ...
     * @returns {jQuery.Deferred<Array<noblecrmerp.web.QueryGroup>> | null}
     */
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of NobleCRMerp records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the NobleCRMerp model this dataset will manage
     */
    ...
    get_id_index: function (id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // NobleCRMerp ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // NobleCRMerp ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, NobleCRMerp domain
     ...
     * an SQL-like form usable by NobleCRMerp's ORM.
     *
     ...
     * NobleCRM's ORM (such as 'name desc', 'product_id', 'order_date asc')
     */
...
var Model = Class.extend({
    /**
    new noblecrmerp.web.Model(name[, context[, domain]])

    @constructs instance.web.Model
    @extends instance.web.Class

    @param {String} name name of the NobleCRMerp model this object is bound to
    @param {Object} [context]
    @param {Array} [domain]
    */
    ...
    /**
     * Call a method (over RPC) on the bound NobleCRMerp model.
     *
