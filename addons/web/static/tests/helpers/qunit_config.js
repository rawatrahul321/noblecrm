 ...
 * QUnit Config
 *
 * The NobleCRM javascript test framework is based on QUnit (http://qunitjs.com/).
 * This file is necessary to setup Qunit and to prepare its interactions with
 * NobleCRM.  It has to be loaded before any tests are defined.
 *
 * Note that it is not an NobleCRM module, because we want this code to be executed
 * as soon as possible, not whenever the NobleCRM module system feels like it.
 */
 ...
 * this is done outside of noblecrm modules, otherwise the setting would not take
 * effect on time.
 */
