...
A very basic NobleCRMerp module structure will be our starting point:
...
There is no such thing as a "web module" declaration. An NobleCRMerp
...
.. note::
    ...
    restart the NobleCRMerp server itself for it to be taken in account.
...
provides: an instance of the NobleCRMerp Web client, with the contents of
...
its :js:func:`~noblecrmerp.web.Widget.start` to add some content to its
DOM:
...
will require restarting the NobleCRMerp Server to see the changes, as
usual):
...
.. note::
    ...
    Prefixing CSS rules with both ``.noblecrmerp`` (to ensure the rule
    will apply only within the confines of the NobleCRMerp Web client) and
...
be done in :js:func:`~noblecrmerp.web.Widget.start` but that gets unwieldy
...
NobleCRMerp Web's template language is :doc:`qweb`. Although any
...
integration to NobleCRMerp Web widgets.
...
To prevent this irremediable loss, we can use NobleCRMerp's support for
...
:js:func:`~noblecrmerp.base.Widget.start()` is to perform *asynchronous*
...
recorded previously using the :js:class:`~noblecrmerp.web.Query` interface
...
4. And finally — after restarting NobleCRMerp — navigating to the test
...
side-effect of this is to :js:func:`~noblecrmerp.web.Widget.start` it,
...
:js:func:`set a widget's DOM <noblecrmerp.web.Widget.setElement>`, let's
