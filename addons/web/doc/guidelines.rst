...
example in NobleCRMerp is dashboards), queries should be restricted to a
...
NobleCRMerp Web's :js:class:`~noblecrmerp.web.Widget` has an attribute
providing its DOM root :js:attr:`Widget.$el <noblecrmerp.web.Widget.$el>`,
and a shortcut to select nodes directly :js:attr:`Widget.$
<noblecrmerp.web.Widget.$>`.
...
* All interactive components (components displaying information to the
  ...
  :class:`~noblecrmerp.web.Widget` and correctly implement and use its API
  and lifecycle.
...
From v11, we introduce a new coding standard for NobleCRM Javascript code.  Here it
is:

* add "use strict"; on top of every noblecrm JS module
