...
Client actions are the client-side version of NobleCRMerp's "Server
...
In the NobleCRMerp Web code, a client action only requires two pieces of
information:
...
* Providing said object. Two different types of objects can be mapped
  ...
  * An NobleCRMerp Web widget, which must inherit from
    :js:class:`noblecrmerp.web.Widget`
...
* if, on the other hand, the client action maps to a
  :js:class:`~noblecrmerp.web.Widget`, that
  :js:class:`~noblecrmerp.web.Widget` will be instantiated and added to
  the web client's canvas, with the usual
  :js:class:`~noblecrmerp.web.Widget` lifecycle (essentially, it will
...
object::
    ...
    // Registers the object 'noblecrmerp.web_dashboard.Widget' to the client
...
At this point, the generic :js:class:`~noblecrmerp.web.Widget` lifecycle
...
should be performed in the :js:func:`~noblecrmerp.web.Widget.start()`
method.

.. note::

    As required by :js:class:`~noblecrmerp.web.Widget`'s contract, if
    :js:func:`~noblecrmerp.web.Widget.start()` executes any asynchronous
