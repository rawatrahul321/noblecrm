...
from noblecrm import api, fields, models, tools, SUPERUSER_ID, _
from noblecrm.exceptions import UserError, AccessError, ValidationError
    ...
    def message_get_email_values(self, notif_mail=None):
        ...
        if res.get('headers'):
            try:
                ...
                pass
        if self.project_id:
            current_objects = [h for h in headers.get('X-NobleCRM-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-NobleCRM-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-NobleCRM-Tags'] = ','.join(self.tag_ids.mapped('name'))
