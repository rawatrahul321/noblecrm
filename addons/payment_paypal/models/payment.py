...
from noblecrm.tools.float_utils import float_compare
...
class TxPaypal(models.Model):
    ...
    def _paypal_form_get_invalid_parameters(self, data):
        ...
        if not data.get('receiver_id') or not self.acquirer_id.paypal_seller_account:
            ...
            # In NobleCRM, there is only one field for the Paypal email: the business email. This isn't possible to set a receiver_email
            # different than the business email. Therefore, if you want such a configuration in your Paypal, you are then obliged to fill
            # the Merchant ID in the Paypal payment acquirer in NobleCRM, so the check is performed on this variable instead of the receiver_email.
