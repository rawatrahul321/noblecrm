noblecrm.define('web_settings_dashboard', function (require) {
"use strict";
...
var DashboardApps = Widget.extend({
    ...
    start: function() {
        this._super.apply(this, arguments);
        if (noblecrm.db_info && _.last(noblecrm.db_info.server_version_info) !== 'e') {
    ...
    confirm_upgrade: function() {
        framework.redirect("https://www.infonoble.com/noblecrm-enterprise/upgrade?num_users=" + (this.data.enterprise_users || 1));
    },
});
...
var DashboardShare = Widget.extend({
    ...
    init: function(parent, data){
        this.data = data;
        this.parent = parent;
        this.share_url = 'https://www.infonoble.com';
        this.share_text = encodeURIComponent("I am using #NobleCRM - Awesome Erp software business apps.");
    },
    ...
    share_linkedin: function(){
        var popup_url = _.str.sprintf('http://www.linkedin.com/shareArticle?mini=true&url=%s&title=I am using noblecrm&summary=%s&source=www.infonoble.com', encodeURIComponent(this.share_url), this.share_text);
        this.sharer(popup_url);
    },
