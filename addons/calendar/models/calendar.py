...
from noblecrm.tools import DEFAULT_SERVER_DATE_FORMAT, DEFAULT_SERVER_DATETIME_FORMAT, pycompat
...
class Meeting(models.Model):
    ...
    hv1 = fields.Selection([('done', 'Done'), ('cancel', 'Cancel'), ('reschedule', 'Reschedule'), ('next','Next Visit')], string='Home Visit 1', track_visibility='onchange')
    hv_remarks = fields.Text('HV Remarks')
        ...
        for meeting in self:
            ...
            if meeting.alarm_ids:
                for alarm in meeting.alarm_ids:
                    ...
                    trigger.value = delta
                    valarm.add('DESCRIPTION').value = alarm.name or u'NobleCRM'
    ...
    def _rrule_parse(self, rule_str, data, date_start):
        ...
        #repeat yearly but for noblecrm it's monthly, take same information as monthly but interval is 12 times
        if rule._bymonth:
