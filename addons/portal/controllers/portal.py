...
from noblecrm import fields as noblecrm_fields, tools, _
from noblecrm.osv import expression
from noblecrm.exceptions import ValidationError
from noblecrm.http import Controller, request, route
from noblecrm.addons.web.controllers.main import WebClient
...
class CustomerPortal(Controller):
    ...
    def _get_archive_groups(self, model, domain=None, fields=None, groupby="create_date", order="create_date desc"):
        ...
        for group in request.env[model]._read_group_raw(domain, fields=fields, groupby=groupby, orderby=order):
            ...
            groups.append({
                'date_begin': noblecrm_fields.Date.to_string(noblecrm_fields.Date.from_string(date_begin)),
                'date_end': noblecrm_fields.Date.to_string(noblecrm_fields.Date.from_string(date_end)),
                'name': label,
