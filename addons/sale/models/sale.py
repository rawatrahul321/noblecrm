...
from noblecrm.exceptions import UserError, AccessError
from noblecrm.osv import expression
from noblecrm.tools import float_is_zero, float_compare, DEFAULT_SERVER_DATETIME_FORMAT

from noblecrm.tools.misc import formatLang

from noblecrm.addons import decimal_precision as dp
...
class SaleOrder(models.Model):
    ...
    @api.depends('payment_lines.deposit_amount','payment_lines.cheque_status')
    def _amount_deduct(self):
        for deposit in self:
            total_deposit = 0.0
            for line in deposit.payment_lines:
                if(line.cheque_status=='deposited' or line.payment_option.name == 'Cash'):
                    total_deposit += line.deposit_amount
            deposit.update({
                'amount_deduct':deposit.amount_total - total_deposit,
                'total_collection':total_deposit
    ...
    partner_id = fields.Many2one(related='crm.partner_id', string='Customer', readonly=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, required=True, index=True, track_visibility='always',store=True)
    crm = fields.Many2one('crm.lead', string='CRM Lead', readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, required=True,
                          change_default=True, index=True, track_visibility='always')

    address = fields.Char(related='crm.street', readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, string='Address',store=True)
    state_id = fields.Many2one(related='crm.state_id', readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, string="State",store=True)
    crn_no = fields.Char("CIRN NO",track_visibility='onchange',store=True)
    crn_no_store = fields.Char(related='crn_no', readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, store=True)
    kit_hand_over = fields.Selection([('yes','Yes'),('no','No')],string="Kit Hand Over",default = 'no')
    kit_handover_date = fields.Date("Kit Hand Over Date")
    kit_box_id = fields.Char("Kit Box Id")
    payment_option = fields.Many2one('account.journal', readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, string='Payment Journal',domain=[('type', 'in', ('bank', 'cash'))])

    Additional_Benefit = fields.Char('additonal benefit')
    campaign_offer = fields.Char('Campaign offer')

    city = fields.Char(related='crm.city', readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, string='Delivery City',store=True)
    phone = fields.Char(related='crm.phone', readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, required=True, string='Phone', store=True)
    a_p = fields.Char(string='Alternate Phone 1', readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, store=True)
    a_p1 = fields.Char(string='Alternate Phone 2', readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, store=True)
    email = fields.Char(related='crm.email_from', readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, string='Email',store=True)
    pincode = fields.Integer('Pin Code' ,readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    otp = fields.Char('OTP',required=True, default="0")
    otp1 = fields.Char('OTP1')
    # state = fields.Char('State')
    doctor = fields.Many2one('res.partner', string="Delivery Doctor",domain="[('contact_type', '=', 'doctor')]",store=True, index=True)
    hospital = fields.Many2one('res.partner', string="Delivery Hospital", domain="[('contact_type', '=', 'hospital')]",store=True, index=True)
    edd = fields.Date('Expected Delivery Date', default=fields.Date.context_today)
    p_t = fields.Selection([('1', 'Single'), ('2', 'Double'), ('3', 'Triple'), ('0', 'Multiple')],
        default="1", string="Pregnancy Type")
    adhar_card = fields.Integer('Adhar Card',readonly=True,
                          states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    pan_card = fields.Char('Pan Card')
    registration_fee = fields.Monetary('Registration Fee')
    payment_lines = fields.One2many('payment.detail','txn_id',string='Payment Lines')
    total_collection = fields.Monetary("Total Collection",readonly=True, compute='_amount_deduct', track_visibility='always')
    telephone = fields.Char('Telephone Bill')
    snapshot = fields.Binary('Snapshot')
    husband_name = fields.Char('Husband Name')
    father_name = fields.Char('Father Name')
    client_identity = fields.Selection([('pan_card','Pan Card'),('adhar_card','Adhar Card'),('telephone','Telephone')],default='adhar_card',track_visibility=True)
    ...
    amount_deduct = fields.Float(string = "amount", store=True,track_visibilit='always',compute='_amount_deduct')
    ...
    def generate(self):
        ...
        self.otp1 = OTP1
        fromaddr = "rahul.rawat@infonoble.com"
        if self.email:

            email=self.email
            contact_name=str(self.partner_id.name)
            ...
            msg['From'] = fromaddr
            msg["To"] = toaddr
            msg["Subject"] = "OTP details"

            body = "Dear "+contact_name+",\n""This email contains OTP(One Time Password)\n""OTP of 6 digits:"+self.otp1+"\n""Please do not share with unauthorized"
            msg.attach(MIMEText(body, 'plain'))
            s = smtplib.SMTP('smtp.gmail.com', 587)
            s.starttls()
            s.login(fromaddr, "krishna321")
            text = msg.as_string()
    ...
    def verify(self):
        a=""
        if self.otp1!=self.otp:
            self.otp=a
            
            return self.otp
            raise UserError('Invalid One Time Password')
    ...
    def submit(self):
        merge_id=[]
        print("--------------inside ---------ompute_cirn---------")
        records = self.env['crm.lead'].search([('id','=',self.crm.id)])
        print("-------------- ---------records %r---------",records)
        code1 = self.env['res.country.state'].search([('id','=',self.state_id.id)])
        print("-------------- ---------code1 %r---------",code1)
        for (record,c_id) in zip(records,code1):
            re=10000+record.id
            r=record.user_code      
            create_date=str(record.create_date)
            c=create_date[2:4]

            record.res=c+str(r)+str(c_id.code)+str(re)
            merge_id.append(record.res)
        for (i,j) in zip(self,merge_id):
            print("i:",i,"j:",j)
            i.crn_no = j
            ...
            if self.email:
                ...
                msg['From'] = fromaddr
                msg["To"] = email
                msg["Subject"] = " :: Contract :"+str(contact_name)
                body = """<pre>
                <p style="font-family:calibri;font-size:12px;">
                <b>Dear """+contact_name+""",</b>
                
                Thank you for choosing Mycord as a preferred banking partner for your baby's precious stem cells . Your Customer Reference Number (CIRN) Is """+i.crn_no+"""

                We are pleased to share your cord blood banking e-contract along with the reference document for digitally signing the agreement .Please click the link below and use the e-Signature pad for digitally signing the agreement.
                
                </p> <a href="http://13.233.17.231/signature/index.php?custid='"""+str(self.id)+"""'&custnonse='101'&custname='"""+contact_name+"""'&custfather='"""+self.father_name+"""'&custresidence='"""+self.address+"""'&custwhatsapp='"""+self.phone+"""'&custspouse='"""+self.husband_name+"""'&CRMnumber='"""+i.crn_no+"""'&enrolmentdate='"""+str(self.create_date)+"""'&custemail='"""+self.email+"""'&custphone='"""+self.phone+"""'&custconname='Client Copy Color.pdf'&contractstatus='submmited'">click here</a>  <p style="font-family:calibri;font-size:12px;">
                <b>Regards,
                Cellugen</b>
                </p>
                """
    ...
    def payment(self):
        ...
        course_form = self.env.ref('account.view_account_payment_invoice_form', False)
...
class SaleOrderLine(models.Model):
    ...
    def _compute_invoice_status(self):
        ...
        for line in self:
            if line.state not in ('sale', 'done'):
                ...
                line.invoice_status = 'no'
    @api.one
    @api.depends('product_uom_qty', 'discount', 'price_unit','discount_amount', 'tax_id')
    def _compute_amount(self):
        ...
        for line in self:
            line.update(
                {
                    'price_subtotal':self.price_subtotal - self.discount_amount
                }
                       )
...
class PaymentDetail(models.Model):
    ...
    _inherit = ['mail.thread','mail.activity.mixin','portal.mixin', 'utm.mixin', 'format.address.mixin'] 
    ...
    txn_id = fields.Many2one("sale.order","payment reference",required=True, ondelete='cascade', index=True, copy=False)
    cirn_no = fields.Char(string="CIRN",store=True,track_visibility='onchange')
    bank_name = fields.Char("Bank Name",track_visibility='always')
    deposit_date = fields.Date("Date",track_visibility='onchange')
    cheque_number = fields.Char("Chq No/Txn Id",track_visibility='always')
    deposit_amount = fields.Float("Amount",store=True,track_visibility='always')
    payment_option = fields.Many2one('account.journal', string='Mode Of Payment',domain=[('type', 'in', ('bank', 'cash'))],required=True,track_visibility='always')
    payment_ref = fields.Selection([('registration','Registration'),('balanced','Balance'),('emi','EMI')],string='Payment Ref.', required=True, default='registration',track_visibility='onchange')
    amount_deduct = fields.Float(related='txn_id.amount_deduct',string="amount", store=True, track_visibilit='always')
    next_due_date = fields.Date(related = "txn_id.next_due_date",string = 'Next Due Date',track_visibility='always')
    amount_total = fields.Monetary(related = "txn_id.amount_untaxed",track_visibility='onchange')
    payment_term_id = fields.Many2one(related = "txn_id.payment_term_id",string="Payment Plan",track_visibility='onchange')
    total_collection = fields.Monetary(related = "txn_id.total_collection")
    cheque_status = fields.Selection(cheque_stats,string='Cheque Status',default='collected')

    #cheque_type = fields.Selection([('pdc_cheque','PDC Cheque'),('cdc_cheque','CDC Cheque')],string="Cheque Type",default="cdc_cheque")
