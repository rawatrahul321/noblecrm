...
from noblecrm.exceptions import AccessError, except_orm
...
class TestMailMessage(TestMail):
    ...
    def test_mail_message_values_document_no_alias(self):
        ...
        self.assertIn('-noblecrmerp-%d-mail.test' % self.test_pigs.id, msg.message_id, 'mail_message: message_id for a void message should be a "private" one')
        self.assertEqual(msg.reply_to, '%s <%s>' % (self.user_employee.name, self.user_employee.email))
        self.assertEqual(msg.email_from, '%s <%s>' % (self.user_employee.name, self.user_employee.email))
...
class TestMailMessageAccess(TestMail):
    ...
    @mute_logger('noblecrm.addons.base.ir.ir_model', 'noblecrm.models')
    ...
    @mute_logger('noblecrm.addons.base.ir.ir_model')
    ...
    @mute_logger('noblecrmerp.addons.mail.models.mail_mail')
