noblecrm.define('mail.chat_manager', function (require) {
"use strict";
...
var NOBLECRMBOT_ID = "NOBLECRMBOT";
...
function notify_incoming_message (msg, options) {
    if (bus.is_noblecrm_focused() && options.is_displayed) {
        // no need to notify
        return;
    }
...
function make_message (data) {
    ...
    if ((!msg.author_id || !msg.author_id[0]) && msg.email_from) {
        ...
        msg.displayed_author = (msg.author_id === NOBLECRMBOT_ID) && "NobleCRMBot" ||
                               msg.author_id && msg.author_id[1] ||
                               msg.email_from || _t('Anonymous');
    }

    // Don't redirect on author clicked of self-posted or NobleCRMBot messages
    msg.author_redirect = !msg.is_author && msg.author_id !== NOBLECRMBOT_ID;

    // Compute the avatar_url
    if (msg.author_id === NOBLECRMBOT_ID) {
        msg.avatar_src = "/mail/static/src/img/noblecrm_o.png";
...
function on_transient_message_notification (data) {
    ...
    data.author_id = data.author_id || NOBLECRMBOT_ID;
    add_message(data);
}
