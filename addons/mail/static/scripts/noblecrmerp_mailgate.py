...
class DefaultConfig(object):
    ...
    """
    NOBLECRMerp_DEFAULT_USER_ID = 1
    NOBLECRMerp_DEFAULT_PASSWORD = 'admin'
    NOBLECRMerp_HOSTNAME = 'localhost'
    NOBLECRMerp_PORT = 8069
    NOBLECRMerp_DEFAULT_DATABASE = 'noblecrmerp'
...
class RPCProxy(object):
    def __init__(self, uid, passwd,
                 host=config.NOBLECRMerp_HOSTNAME,
                 port=config.NOBLECRMerp_PORT,
                 path='object',
                 dbname=config.NOBLECRMerp_DEFAULT_DATABASE):
...
def configure_parser():
    ...
    group = optparse.OptionGroup(parser, "Note",
        ...
        "with the NobleCRM server for case management in the CRM module.")
    parser.add_option_group(group)
    parser.add_option("-u", "--user", dest="userid",
                      help="NobleCRM user id to connect with",
                      default=config.NOBLECRMerp_DEFAULT_USER_ID, type='int')
    parser.add_option("-p", "--password", dest="password",
                      help="NobleCRM user password",
                      default=config.NOBLECRMerp_DEFAULT_PASSWORD)
    ...
    parser.add_option("-d", "--dbname", dest="dbname",
                      help="NobleCRM database name (default: %default)",
                      default=config.NOBLECRMerp_DEFAULT_DATABASE)
    parser.add_option("--host", dest="host",
                      help="NobleCRM Server hostname",
                      default=config.NOBLECRMerp_HOSTNAME)
    parser.add_option("--port", dest="port",
                      help="NobleCRM Server XML-RPC port number",
                      default=config.NOBLECRMerp_PORT)
...
def main():
    """
    Receive the email via the stdin and send it to the NobleCRM Server
    """
    ...
    try:
        ...
        subject = '[NobleCRM]:ERROR: Mailgateway - %s' % time.strftime('%Y-%m-%d %H:%M:%S')
        send_mail(
            config.MAIL_ERROR,
            config.MAIL_ADMINS,
            subject, msg, files=[('message.txt', msg_txt)]
        )
        sys.stderr.write("Failed to deliver email to NobleCRM Server, sending error notification to %s\n" % config.MAIL_ADMINS)
