...
from noblecrm import api, fields, models, tools, SUPERUSER_ID
from noblecrm.tools.translate import _
from noblecrm.tools import email_re, email_split
...
class Lead(models.Model):
    ...
    edd = fields.Date("Expected Delivery Date")
    ...
    def create(self, vals):
        ...
        # country_ids = self.env['res.users'].search([('id','=',self.user_id.id)])
        # print("%%%%%%%%%%%%%%%%%%%%%%%%%%",res)

 
        
           
        # 'user_id':country_ids.user_id, # Many2one field
        # 'rsm': country_ids.area.user_id,
        # 're': self.area.user_id.id,
        # 'asm' : self.region.user_id 
      
        # print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^6",self.area.user_id.id)
        # print("################################",self.area.user_id)
        # return res
