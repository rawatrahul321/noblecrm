...
from noblecrm.addons.base.ir.ir_http import RequestUID, ModelConverter
from noblecrm.http import request
from noblecrm.tools import config, ustr, pycompat
...
def is_multilang_url(local_url, langs=None):
    ...
    try:
        ...
        # Force to check method to POST. NobleCRM uses methods : ['POST'] and ['GET', 'POST']
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if noblecrm._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            noblecrm._geoip_resolver = GeoIPResolver.open(geofile) or False
        except Exception as e:
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if noblecrm._geoip_resolver and request.httprequest.remote_addr:
                record = noblecrm._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except noblecrm.exceptions.MissingError:
