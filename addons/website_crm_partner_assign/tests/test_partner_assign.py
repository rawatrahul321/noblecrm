...
from noblecrm.exceptions import AccessError

from noblecrm.tests.common import TransactionCase
from noblecrm.addons.crm.tests.common import TestCrmCases
...
class TestPartnerAssign(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('noblecrm.addons.base_geolocalize.models.res_partner.geo_find', wraps=geo_find)
        patcher.start()
        self.addCleanup(patcher.stop)

        patcher = patch('noblecrm.addons.website_crm_partner_assign.models.crm_lead.geo_find',
                        wraps=geo_find)
        patcher.start()
        self.addCleanup(patcher.stop)
...
class TestPartnerLeadPortal(TestCrmCases):
    ...
    def setUp(self):
        ...
        self.portal_user = self.env['res.users'].with_context({'no_reset_password': True, 'mail_notrack': True}).create({
            'name': 'Super Customer NobleCRM Intregrating Partner',
