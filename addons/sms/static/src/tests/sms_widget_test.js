noblecrm.define('sms.sms_widget_tests', function (require) {
"use strict";
...
QUnit.module('fields', {
    ...
    QUnit.test('Sms widgets are correctly rendered', function (assert) {
        ...
        // GSM-7
        form.$('.o_input').val("Hello from NobleCRM").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 / 1 SMS (GSM7) ', 'Should be "15 / 1 SMS (GSM7) " for "Hello from NobleCRM"');
        // GSM-7 with \n => this one count as 2 characters
        form.$('.o_input').val("Hello from NobleCRM\n").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '17 / 1 SMS (GSM7) ', 'Should be "17 / 1 SMS (GSM7) " for "Hello from NobleCRM\\n"');
        // Unicode => ê
        form.$('.o_input').val("Hêllo from NobleCRM").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 / 1 SMS (UNICODE) ', 'Should be "15 / 1 SMS (UNICODE) " for "Hêllo from NobleCRM"');
        // GSM-7 with 160c
