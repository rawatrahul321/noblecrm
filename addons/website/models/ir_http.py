...
from noblecrm import SUPERUSER_ID
from noblecrm.http import request
from noblecrm.tools import config
from noblecrm.exceptions import QWebException
from noblecrm.tools.safe_eval import safe_eval
from noblecrm.osv.expression import FALSE_DOMAIN, OR

from noblecrm.addons.http_routing.models.ir_http import ModelConverter, _guess_mimetype
from noblecrm.addons.portal.controllers.portal import _build_url_w_params
...
class Http(models.AbstractModel):
    ...
    def _handle_exception(cls, exception):
        ...
        if not is_website_request:
            ...
            if isinstance(exception, noblecrm.exceptions.AccessError):
                code = 403

            if isinstance(exception, QWebException):
                values.update(qweb_exception=exception)
                if isinstance(exception.qweb.get('cause'), noblecrm.exceptions.AccessError):
                    code = 403
